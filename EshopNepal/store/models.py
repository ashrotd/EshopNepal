...
class Product(models.Model):
    ...
    def AverageRating(self):
        reviews = ReviewSystem.objects.filter(product=self, status=True).aggregate(average=Avg('rating'))
        avg = 0
    ...
    def countReview(self):
        reviews = ReviewSystem.objects.filter(product=self, status=True).aggregate(count=Count('rating'))
        count = 0
